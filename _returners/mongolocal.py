...
### MONGO DB CONNECTION FUNCTIONALITY ###
...
def prep_jid(nocache=False, passed_jid=None, recurse_count=0):
    ...
    """
    if recurse_count >= 5:
        err = "prep_jid could not store a jid after {} tries.".format(
            recurse_count)
        log.error(err)
...
def returner(load):
    ...
    if isinstance(load["return"], dict):
        back = _remove_dots(load["return"])
    else:
        back = load["return"]

    if isinstance(load, dict):
        full_ret = _remove_dots(load)
    else:
        full_ret = load
    ...
    outdata = {"$set": sdata}
    mdb.jobs.update_one({"jid": load["jid"]}, {"$set": outdata}, upsert=True)
    ...
    jid_dir = salt.utils.jid.jid_dir(
        load["jid"], _job_dir(), __opts__["hash_type"])
    ...
    salt.payload.dump(
        {key: load[key]
            for key in ["return", "retcode", "success"] if key in load},
...
def save_minions(jid, minions, syndic_id=None):
    ...
    if syndic_id is not None:
        minions_path = os.path.join(
            jid_dir, SYNDIC_MINIONS_P.format(syndic_id))
    else:
...
def get_load(jid):
    ...
    minions_cache.extend(glob.glob(os.path.join(
        jid_dir, SYNDIC_MINIONS_P.format("*"))))
    all_minions = set()
